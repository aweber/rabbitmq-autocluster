...
-define(CONSUL_REGISTER_BODY, "{\"name\":\"" ++ ?CONSUL_SERVICE ++ "\"}").
...
init() ->
  ...
  case cluster_nodes() of
    ...
    [DiscoveryNode|_] ->
      ...
      mnesia:stop(),
      rabbit_mnesia:reset(),
      io:format("Discovery nodes: ~p~n", [DiscoveryNode]),
...
%%
cluster_nodes() ->
  case httpc:request(build_url("catalog/service", ?CONSUL_SERVICE)) of
    {ok, {{_, 200, _}, _, Body}} ->
